...
#pragma HLS AGGREGATE compact=auto variable=buf.val
  ...
  TENSOR_WEIGHT_Y_OUTER: for(int r=0; r<MAX_HEIGHT+1; r++)
  {
    TENSOR_WEIGHT_Y_INNER: for(int c=0; c<MAX_WIDTH; c++)
    {
      ...
      outer_t tmp;
#pragma HLS AGGREGATE compact=auto variable=tmp.val
      buf.shift_pixels_up(c);
      if(r<MAX_HEIGHT)
      {
        tmp = outer[r][c];
...
#pragma HLS DATAFLOW
	...
	#pragma HLS STREAM variable=outputs depth=default_depth


	// stream in and organize the inputs
	static frames_t buf;
	FRAMES_CP_OUTER: for (int r=0; r<MAX_HEIGHT; r++) {
		FRAMES_CP_MID: for (int c=0; c<MAX_WIDTH/8; c++) {
			// one wide read
			ap_uint<512> in_tmp;
			in_tmp = Input_1.read();
			FRAMES_CP_INNER: for(int v=0; v<8; v++){
			#pragma HLS pipeline II=1
				buf(63, 0) = in_tmp(64*v+63, v*64);
				// assign values to the FIFOs
				frame1_a[r][c*8+v] = ((input_t)(buf(7 ,  0)) >> 8);
				frame2_a[r][c*8+v] = ((input_t)(buf(15,  8)) >> 8);
				frame3_a[r][c*8+v] = ((input_t)(buf(23, 16)) >> 8);
				frame3_b[r][c*8+v] = ((input_t)(buf(23, 16)) >> 8);
				frame4_a[r][c*8+v] = ((input_t)(buf(31, 24)) >> 8);
				frame5_a[r][c*8+v] = ((input_t)(buf(39, 32)) >> 8);
			}
		}
	}
	...
	OUT_OUTER: for (int r=0; r<MAX_HEIGHT; r++) {
		OUTER_INNER: for (int c=0; c<MAX_WIDTH/8; c++) {
			ap_uint<512> out_tmp;
			// one wide read
			for(int v=0; v<8; v++) {
#pragma HLS pipeline II=1
				out_tmp(64*v+31,    v*64) = outputs[r*MAX_WIDTH+c*8+v].x.range(31,0);
				out_tmp(64*v+63, v*64+32) = outputs[r*MAX_WIDTH+c*8+v].y.range(31,0);
				// printf("output: %d\n", r*MAX_WIDTH+c*8+v);
			}
...
#pragma HLS INTERFACE axis register both port=Output_1
  ...
  FLOW_OUTER: for(int r=0; r<MAX_HEIGHT; r++)
  {
    ...
    {
      #pragma HLS pipeline II=1
      ap_uint<512> in_tmp = Input_1.read();

      if(r>=2 && r<MAX_HEIGHT-2 && c>=2 && c<MAX_WIDTH-2)
      {
	      calc_pixel_t t1 = (calc_pixel_t) in_tmp(48*0+47, 48*0);
	      calc_pixel_t t2 = (calc_pixel_t) in_tmp(48*1+47, 48*1);
	      calc_pixel_t t3 = (calc_pixel_t) in_tmp(48*2+47, 48*2);
	      calc_pixel_t t4 = (calc_pixel_t) in_tmp(48*3+47, 48*3);
	      calc_pixel_t t5 = (calc_pixel_t) in_tmp(48*4+47, 48*3);
	      calc_pixel_t t6 = (calc_pixel_t) in_tmp(48*5+47, 48*5);
      ...
      ap_uint<512> out_tmp;
      vel_pixel_t out1 = (vel_pixel_t)buf[0];
      vel_pixel_t out2 = (vel_pixel_t)buf[1];
      out_tmp(31,  0) = out1(31, 0);
      out_tmp(63, 32) = out2(31, 0);
      Output_1.write(out_tmp);
      //printf("c:%d\n", i++);
    }
  }
