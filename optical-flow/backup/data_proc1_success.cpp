...
void data_proc1(hls::stream< ap_uint<512> > & Input_1, hls::stream<ap_uint<512> > & Output_1) {
	...
	FRAMES_CP_OUTER: for (int r=0; r<MAX_HEIGHT; r++) {
		FRAMES_CP_MID: for (int c=0; c<MAX_WIDTH/8; c++) {
			...
			ap_uint<512> in_tmp;
			in_tmp = Input_1.read();
			FRAMES_CP_INNER: for(int k=0; k<8; k++) {
        		#pragma HLS pipeline II=1
				buf(63, 0) = in_tmp(64*k+63, 64*k);
				// assign values to the FIFOs
				frame1_a[r][c*8+k] = ((input_t)(buf(7 ,  0)) >> 8);
				frame2_a[r][c*8+k] = ((input_t)(buf(15,  8)) >> 8);
				frame3_a[r][c*8+k] = ((input_t)(buf(23, 16)) >> 8);
				frame3_b[r][c*8+k] = ((input_t)(buf(23, 16)) >> 8);
				frame4_a[r][c*8+k] = ((input_t)(buf(31, 24)) >> 8);
				frame5_a[r][c*8+k] = ((input_t)(buf(39, 32)) >> 8);
			}
		}
	}
