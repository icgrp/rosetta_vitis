...
CXXFLAGS += -I$(SYSROOT)/usr/include/xrt -I$(XILINX_HLS)/include -Wall -O0 -g -std=c++1y
...
VPP_FLAGS += -t $(TARGET) --platform $(PLATFORM) --save-temps --vivado.synth.jobs $(shell nproc) --vivado.impl.jobs $(shell nproc) 
...
$(TEMP_DIR)/increment.xo: ../host/top.cpp $(OPERATORS_SRC)
	mkdir -p $(TEMP_DIR)
	v++ $(VPP_FLAGS) -c -k increment --temp_dir $(TEMP_DIR) -D$(ACCEL) -I'$(<D)' -o'$@' $^
...
$(BUILD_DIR)/krnl_incr.xclbin: $(TEMP_DIR)/mem_read1.xo $(TEMP_DIR)/mem_read2.xo $(TEMP_DIR)/mem_read3.xo $(TEMP_DIR)/increment.xo $(TEMP_DIR)/mem_write1.xo $(TEMP_DIR)/mem_write2.xo $(TEMP_DIR)/mem_write3.xo
	mkdir -p $(BUILD_DIR)
	v++ $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) $(VPP_LDFLAGS_krnl_incr) -o'$(LINK_OUTPUT)' $(+)


############################## Setting Rules for Host (Building Host Executable) ##############################
$(EXECUTABLE): $(HOST_SRCS) $(img_obj) | check-vitis check_edge_sw
	$(XILINX_VITIS)/gnu/aarch64/lin/aarch64-linux/bin/aarch64-linux-gnu-g++ -o $@ $^ $(CXXFLAGS) $(LDFLAGS)

$(img_obj):$(SRC_DIR)/%.o:$(SRC_DIR)/%.cpp 
	$(XILINX_VITIS)/gnu/aarch64/lin/aarch64-linux/bin/aarch64-linux-gnu-g++ -Wall -g -std=c++11 -c $^ -o $@
