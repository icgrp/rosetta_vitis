...
void datamover (
		hls::stream< ap_axiu<512, 0, 0, 0> > & Input_1,
		hls::stream< ap_axiu<512, 0, 0, 0> > & Output_1) {
...
#pragma HLS DATAFLOW
	...
	FRAMES_CP_OUTER: for (int r=0; r<MAX_HEIGHT; r++) {
		FRAMES_CP_MID: for (int c=0; c<MAX_WIDTH/8; c++) {
			// one wide read
			ap_axiu<512, 0, 0, 0> in_tmp;
			in_tmp = Input_1.read();
			FRAMES_CP_INNER: for(int v=0; v<8; v++){
			#pragma HLS pipeline II=1
				buf(63, 0) = in_tmp.data(64*v+63, v*64);
	...
	OUT_OUTER: for (int r=0; r<MAX_HEIGHT; r++) {
		OUTER_INNER: for (int c=0; c<MAX_WIDTH/8; c++) {
			ap_axiu<512, 0, 0, 0> out_tmp;
			// one wide read
			for(int v=0; v<8; v++) {
#pragma HLS pipeline II=1
				out_tmp.data(64*v+31,    v*64) = outputs[r*MAX_WIDTH+c*8+v].x.range(31,0);
				out_tmp.data(64*v+63, v*64+32) = outputs[r*MAX_WIDTH+c*8+v].y.range(31,0);
				// printf("output: %d\n", r*MAX_WIDTH+c*8+v);
			}
