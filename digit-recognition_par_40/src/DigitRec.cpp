...
extern "C" {
	void DigitRec(
			WholeDigitType *global_training_set1,
			WholeDigitType *global_training_set2,
			WholeDigitType *global_test_set,
			LabelType *global_results,
			int run
	        )
	{
#pragma HLS INTERFACE m_axi port=global_training_set1 bundle=aximm1
#pragma HLS INTERFACE m_axi port=global_training_set2 bundle=aximm2
#pragma HLS INTERFACE m_axi port=global_test_set bundle=aximm3
#pragma HLS INTERFACE m_axi port=global_results bundle=aximm1
		  ...
		  for (int i = 0; i < NUM_TRAINING / 2; i ++ ){
		    ...
		    training_set[i]                    = global_training_set1[i];
		    training_set[i + NUM_TRAINING / 2] = global_training_set2[i];
		  }
