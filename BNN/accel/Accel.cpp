...
extern "C" {
  ...
  {
#pragma HLS INTERFACE m_axi port=wt_i       bundle=aximm1
#pragma HLS INTERFACE m_axi port=kh_i       bundle=aximm2
#pragma HLS INTERFACE m_axi port=data_i     bundle=aximm3
#pragma HLS INTERFACE m_axi port=sched_list bundle=aximm4
#pragma HLS INTERFACE m_axi port=data_o     bundle=aximm5

	for(int sched_index=0; sched_index<54; sched_index++){
	    const Address    n_inputs = sched_list[sched_index*8];
	    const Address    n_outputs = sched_list[sched_index*8+1];
	    const Address    input_words = sched_list[sched_index*8+2];
	    const Address    output_words = sched_list[sched_index*8+3];
	    const ap_uint<3> layer_mode = sched_list[sched_index*8+4];  // [0]='new layer', [2:1]='conv1,conv,dense,last'
	    const ap_uint<1> dmem_mode = sched_list[sched_index*8+5];   // 0 means dmem[0] is input
	    const ap_uint<2> width_mode = sched_list[sched_index*8+6];  // 0=8'b, 1=16'b, 2=32'b
	    const ap_uint<2> norm_mode = sched_list[sched_index*8+7];    // 0='do nothing', 1='do norm', 2='do pool'

        top(
          wt_i+4682*sched_index,
		  kh_i+64*sched_index,
		  data_i,
		  data_o,
		  n_inputs,
