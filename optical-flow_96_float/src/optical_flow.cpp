...
{
// void tensor_weight_x(outer_t tensor_y[MAX_HEIGHT][MAX_WIDTH],
//                      tensor_t tensor[MAX_HEIGHT][MAX_WIDTH])
// {
  ...
  TENSOR_WEIGHT_X_OUTER: for(int r=0; r<MAX_HEIGHT; r++)
  {
    TENSOR_WEIGHT_X_INNER: for(int c=0; c<MAX_WIDTH+1; c++)
    {
      ...
      {
        // for(int i=0;i<6;i++){
        //   tmp.val[i] = tensor_y[r][c].val[i]; // added
        // }
        tmp = tensor_y[r][c];
      }
      else
               ...
               velocity_t outputs[MAX_HEIGHT*MAX_WIDTH])
{
  ...
  FLOW_OUTER: for(int r=0; r<MAX_HEIGHT; r++)
  {
    FLOW_INNER: for(int c=0; c<MAX_WIDTH; c++)
    {
      ...
      outputs[r*MAX_WIDTH+c].x = tmpvel_0;
      outputs[r*MAX_WIDTH+c].y = tmpvel_1;
...
extern "C" {
			...
			int run)
	{
		...
		{
          //printf("r=%d: %08x%_%08x\n", r*MAX_WIDTH, (unsigned int) frames[r*MAX_WIDTH](63, 32), (unsigned int) frames[r*MAX_WIDTH](31,  0));
