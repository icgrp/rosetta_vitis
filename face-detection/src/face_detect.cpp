...
extern "C" {
	void face_detect(
			  ...
			  int result[RESULT_SIZE*4+1],
			  int run
	        )
	{
#pragma HLS INTERFACE m_axi port=inData bundle=aximm1
#pragma HLS INTERFACE m_axi port=result bundle=aximm1
	...
	#pragma HLS pipeline II=1
	  ...
	  while ( IMAGE_WIDTH/factor > WINDOW_SIZE && IMAGE_HEIGHT/factor > WINDOW_SIZE )
	  {
	   ...
	   for ( i = 0; i < RESULT_SIZE; i++){
		  result[i]                 = result_x_Scale[i];
		  result[RESULT_SIZE   + i] = result_y_Scale[i];
		  result[RESULT_SIZE*2 + i] = result_w_Scale[i];
		  result[RESULT_SIZE*3 + i] = result_h_Scale[i];
	   }
	   result[400] = *result_size_Scale;
	}
}
